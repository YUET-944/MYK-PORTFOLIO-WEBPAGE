-- Complete Supabase Database Setup for Muhammad Younas Khan Portfolio
...
INSERT INTO profile (name, title, bio, email, phone, location, github_url, linkedin_url, skills, languages, roles) 
SELECT 
...
-- Insert default projects (only if not exists)
INSERT INTO projects (title, description, full_description, technologies, status, type, github_url, gradient, challenges, learned, timeline, is_featured, order_index) 
SELECT * FROM (VALUES 
  (
    'Social Media MVP',
  ...
  (
    ...
    'Modern, responsive portfolio website built with Next.js, React, and Tailwind CSS featuring dynamic content management.',
    'A full-stack portfolio website with admin panel, contact form, review system, and database integration. Features include responsive design, dark mode, email notifications, and content management system.',
    ARRAY['Next.js', 'React', 'Tailwind CSS', 'Supabase', 'TypeScript'],
    'completed',
    'personal',
    'https://github.com/YUET-944/muhammad-younas-portfolio',
    'from-indigo-400 to-emerald-600',
    ARRAY['Full-stack integration', 'Database design', 'Email automation'],
    ARRAY['Next.js development', 'Database management', 'Deployment'],
    '2 months',
    true,
    3
  )
) AS new_projects(title, description, full_description, technologies, status, type, github_url, gradient, challenges, learned, timeline, is_featured, order_index)
WHERE NOT EXISTS (SELECT 1 FROM projects WHERE title = new_projects.title);

-- Insert a sample approved review
INSERT INTO reviews (name, role, company, email, rating, review, status)
SELECT 
  'Abdul Muhamin',
  ...
  'approved'
WHERE NOT EXISTS (SELECT 1 FROM reviews WHERE name = 'Abdul Muhamin');

-- Verify the setup
SELECT 'Projects created:' as info, count(*) as count FROM projects
UNION ALL
SELECT 'Profile created:', count(*) FROM profile
UNION ALL
